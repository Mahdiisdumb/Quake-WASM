...
// An unprecached setmodel goes through host_late_precache_model (QSS late precache);
// -1 back (extension system off) keeps the vanilla fatal as unreachable().
// NOT modeled: setsize's backwards-mins/maxs guard.
//
...
// Host rescue for an unprecached setmodel (QSS PF_sv_setmodel late precache):
// registers the model host-side AND in this registry, returns the slot; -1 = vanilla fatal.
declare function host_late_precache_model(nameOfs: i32): i32;
...
export function pf_setmodel(g: usize): void {
  ...
  if (idx == -1) idx = host_late_precache_model(nameOfs);
