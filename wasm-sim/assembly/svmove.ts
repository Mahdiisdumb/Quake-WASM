...
export function rebuildSkinContents(numEdicts: i32): void {
  ...
  for (let e: i32 = 1; e < numEdicts; ++e) {
    ...
    // SOLID_BSP brushes and negative-skin SOLID_TRIGGER boxes (FTE sv_user.c:7057), as sv.ts
    const solid: i32 = <i32>edLoadFloat(e, F_SOLID);
    if (solid != SOLID_BSP && solid != SOLID_TRIGGER) continue;
...
export function skinContentsAt(
  ...
  for (let i: i32 = 0; i < skinListCount; ++i) {
    ...
    const tsolid: i32 = <i32>edLoadFloat(touch, F_SOLID);
    if (tsolid != SOLID_BSP && tsolid != SOLID_TRIGGER) continue;
    const skin: i32 = <i32>edLoadFloat(touch, F_SKIN);
    if (skin >= 0) continue;
