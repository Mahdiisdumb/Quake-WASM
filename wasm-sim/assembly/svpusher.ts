...
function pushMoveLinear(pusherNum: i32, movetime: f64): void {
  ...
  for (let check: i32 = 1; check < numEdicts; check++) {
    ...
    if (testEntityPosition(check)) {
      // Flush-rider precision nudge — sv.ts pushMove (FTE WPhys_Push). Op order must match.
      const nudgeLen: f64 = Math.sqrt(moveX * moveX + moveY * moveY + moveZ * moveZ);
      if (nudgeLen != 0.0) {
        const nudge: f64 = 0.125 / nudgeLen;
        edStoreFloat(check, F_ORIGIN, <f64>edLoadFloat(check, F_ORIGIN) + moveX * nudge);
        edStoreFloat(check, F_ORIGIN1, <f64>edLoadFloat(check, F_ORIGIN1) + moveY * nudge);
        edStoreFloat(check, F_ORIGIN2, <f64>edLoadFloat(check, F_ORIGIN2) + moveZ * nudge);
        if (!testEntityPosition(check)) {
          linkEdict(check);
          continue;
        }
