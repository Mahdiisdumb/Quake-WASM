...
// The bridge is server-side by definition: it reads the ssqc VM, never the active one
// (a client VM can be switched in on the main thread while this runs).
const ssqc = pr.vms.ssqc;
...
function ensureViews() {
  const b = mem();
  if (b === viewBuf) return;
  globalsView = new Int32Array(b, x.globalsPtr(), ssqc.globals_int.length);
...
function stageExtStringArgs(name: string) {
  ...
  const argc = ssqc.argc;
  const heap: any = ssqc.strings;
  for (let i = first; i < argc && i < 8; i++) {
    ...
    stagedSlots.push(slot); stagedVals.push(ssqc.globals_int[slot]);
    ssqc.globals_int[slot] = base;
  }
}
function restoreExtStringArgs() {
  for (let i = 0; i < stagedSlots.length; i++) ssqc.globals_int[stagedSlots[i]] = stagedVals[i];
}
  ...
  return {
    ...
    host: {
      ...
      host_extbuiltin: (n: number) => {
        ...
        ssqc.globals_int.set(globalsView!.subarray(0, K));
        ssqc.argc = x.getArgc();
        let strResult = false;
        switch (n) {
    ...
    builtins_model: {
      // QSS late precache for an unprecached setmodel: append host-side, register the same
      // slot in the sim under the string offset QC passed. Brush-model hulls are NOT
      // installed on this path (box-hull collision only). -1 = keep the vanilla fatal.
      host_late_precache_model: (nameOfs: number) => {
        if (!pr.cvr.pr_checkextension.value) return -1;
        const name = readWasmString(nameOfs);
        con.print('PF_setmodel("' + name + '"): Model was not precached\n');
        const i = pf.precacheModelLate(name);
        const m = sv.state.server.models[i];
        const mins = m != null ? (m.clip_mins != null ? m.clip_mins : m.mins) || [0, 0, 0] : [0, 0, 0];
        const maxs = m != null ? (m.clip_maxs != null ? m.clip_maxs : m.maxs) || [0, 0, 0] : [0, 0, 0];
        const slot = x.registerModelPrecache(nameOfs, mins[0], mins[1], mins[2], maxs[0], maxs[1], maxs[2]);
        if (slot !== i)
          con.print('[sv_wasm] late precache "' + name + '" sim slot ' + slot + ' != server index ' + i + '\n');
        return slot;
      },
    },
...
export async function loadMap() {
  ...
  {
    const jsStrHeap: any = ssqc.strings;
  ...
  startFrameFn = ssqc.globals_int[pr.globalvars.StartFrame];
  ...
  x.initEntState(sv.state.svs.maxclients, ssqc.num_edicts);
  // QSS-M's qcvm->brokenbouncemissile + ->rotatingbmodel. Per map, since the progs decide it.
  x.setRerelease(ssqc.rerelease ? 1 : 0);
  ...
  x.setGravityFieldIdx(ssqc.entvars.gravity != null ? ssqc.entvars.gravity : -1);
  ...
  x.setClientGravityFieldIdx(ssqc.entvars.gravity != null ? ssqc.entvars.gravity : -1);
  ...
  ensureViews();
  seedStateToWasm();
  for (let e = 0; e < ssqc.num_edicts; e++) x.setEdictFree(e, s.edicts[e].free ? 1 : 0);
  ...
  for (let e = 1; e < ssqc.num_edicts; e++) if (s.edicts[e].free !== true) x.linkEdict(e);
  ...
  for (let e = 0; e < ssqc.num_edicts; e++) {
    const vf = s.edicts[e].v_float;
    prevOrigins[e * 3] = vf[ssqc.entvars.origin]; prevOrigins[e * 3 + 1] = vf[ssqc.entvars.origin + 1]; prevOrigins[e * 3 + 2] = vf[ssqc.entvars.origin + 2];
    prevModelindex[e] = vf[ssqc.entvars.modelindex];
  }
  ...
  for (let e = 1; e < ssqc.num_edicts; e++) { const ed = s.edicts[e]; if (ed && ed.free !== true) sv.unlinkEdict(ed); }
  ready = true;
  con.print('[sv_wasm] WASM server backend loaded (' + ssqc.num_edicts + ' edicts)\n');
}
...
function loadProgsIntoWasm(bytes: Uint8Array) {
  ...
  const prFns: any[] = ssqc.functions as any;
...
function installBmodelHulls() {
  ...
  for (let mi = 0; mi < names.length; mi++) {
    const m = models[mi];
    // Mirror pf.setmodel: alias models register their vanilla +/-16 clip box, not render bounds.
    const mins = m != null ? (m.clip_mins != null ? m.clip_mins : m.mins) || [0, 0, 0] : [0, 0, 0];
    const maxs = m != null ? (m.clip_maxs != null ? m.clip_maxs : m.maxs) || [0, 0, 0] : [0, 0, 0];
...
function seedStateToWasm() {
  ensureViews();
  const s = sv.state.server;
  globalsView!.set(ssqc.globals_int.subarray(0, globalsView!.length));
  for (let e = 0; e < ssqc.num_edicts; e++) edictsView!.set(s.edicts[e].v_int, e * entityFields);
}
...
function syncGlobalsOut() {
  ...
  ssqc.globals_int.set(globalsView!.subarray(0, ssqc.globals_int.length));
}
...
function disableBackend(reason: string) {
  ...
  const s = sv.state.server;
  for (let e = 0; e < ssqc.num_edicts; e++) { const ed = s.edicts[e]; if (ed != null) sv.unbindEdictStorage(ed); }
  ...
  for (let e = 1; e < ssqc.num_edicts; e++) { const ed = s.edicts[e]; if (ed != null && ed.free !== true) sv.linkEdict(ed); }
...
export function frame() {
  const s = sv.state.server;
  // host_extbuiltin runs JS pf impls that read the ACTIVE VM; force ssqc.
  pr.switchVM(ssqc);
  try {
    x.setNumEdicts(ssqc.num_edicts);
    ...
    for (let e = 1; e <= sv.state.svs.maxclients; e++) {
      ...
      x.setUserCmd(e, c.cmd.forwardmove, c.cmd.sidemove, c.cmd.upmove,
        vf[ssqc.entvars.v_angle], vf[ssqc.entvars.v_angle + 1], vf[ssqc.entvars.v_angle + 2],
        vf[ssqc.entvars.button0], vf[ssqc.entvars.button2], vf[ssqc.entvars.impulse]);
    }
    ...
    if (x.wasTrapped()) {
      ...
      let cls = '?';
      try { cls = readWasmString(edictsView![self * entityFields + ssqc.entvars.classname]); } catch (_e) { /* ignore */ }
    ...
    const po = prevOrigins!, OX = ssqc.entvars.origin;
    const pm = prevModelindex!, MI = ssqc.entvars.modelindex;
    ...
    ssqc.time = s.time;       // ...and the ssqc VM clock that shadows it (ed.alloc, runThink)
  } catch (e: any) {
