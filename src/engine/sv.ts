	...
	for (; ;) {
		if (node.contents < 0) {
			if (node.contents !== mod.CONTENTS.solid) {
				pvs = mod.leafPVS(node as Leaf, state.server.worldmodel);
...
export const checkBottom = function (ent: Edict): boolean {
	...
	var start: V3 = [(mins[0] + maxs[0]) * 0.5, (mins[1] + maxs[1]) * 0.5, mins[2]];
	var stop: V3 = [start[0], start[1], start[2] - 36.0];
...
export const movestep = async function (ent: Edict, _move: [number, number], relink: boolean) {
	var oldorg = ed.vector(ent, pr.entvars.origin);
	var neworg:V3 = vec.emptyV3();
	...
	var end: V3 = [neworg[0], neworg[1], neworg[2] - 36.0];
...
export const stepDirection = async function (ent: Edict, yaw: number, dist: number) {
...
export const newChaseDir = async function (actor: Edict, enemy: Edict, dist: number) {
...
async function runThink(ent: Edict) {
...
const impact = async function (e1: Edict, e2: Edict) {
...
const flyMove = async function (ent: Edict, time: number) {
	var bumpcount;
	var numplanes = 0;
	var dir, d;
	var planes: V3[] = [], plane;
	...
	var new_velocity: V3 = vec.emptyV3();
	var i, j;
	var trace;
	var end: V3 = vec.emptyV3();
	var time_left = time;
	var blocked = 0;
...
const pushEntity = async function (ent: Edict, push: V3) {
	var end: V3 = [
...
const pushMove = async function (pusher: Edict, movetime: number) {
	...
	type EdictContainer = [number, number, number, Edict];
	var entorig, moved: EdictContainer[] = [], moved_edict: EdictContainer, i;
	for (e = 1; e < state.server.num_edicts; ++e) {
		check = state.server.edicts[e];
		if (check.free === true)
			continue;
...
const physics_Pusher = async function (ent: Edict) {
...
const checkStuck = async function (ent: Edict) {
...
const tryUnstick = async function (ent: Edict, oldvel: V3) {
	var oldorg = ed.vector(ent, pr.entvars.origin);
	var dir: V3 = [2.0, 0.0, 0.0];
	var i, clip;
	for (i = 0; i <= 7; ++i) {
		switch (i) {
...
const walkMove = async function (ent: Edict) {
...
const physics_Client = async function (ent: Edict) {
	try {
...
// const physics_Noclip = async function (ent: Edict) {
// 	if (await runThink(ent) !== true)
// 		return;
...
const physics_Toss = async function (ent: Edict) {
	if (await runThink(ent) !== true)
		return;
...
const physics_Step = async function (ent: Edict) {
...
const setIdealPitch = function () {
	...
	var top: V3 = [0.0, 0.0, ent.v_float[pr.entvars.origin2] + ent.v_float[pr.entvars.view_ofs2]];
	var bottom: V3 = [0.0, 0.0, top[2] - 160.0];
	var i, tr, z = [];
	for (i = 0; i < 6; ++i) {
...
const airMove = function () {
	...
	var wishdir: V3 = [wishvel[0], wishvel[1], wishvel[2]],
...
const createAreaNode = function (depth: number, mins: V3, maxs: V3) {
	var anode = {} as any;
...
const touchLinks = async function (ent: Edict, node: AreaNode) {
	var list: Edict[] = [], old_self, old_other
...
export const linkEdict = async function (ent: Edict, touch_triggers: boolean = false) {
...
const clipMoveToEntity = function (ent: Edict, start: V3, mins: V3, maxs: V3, end: V3) {
	var trace: Trace = {
		fraction: 1.0,
		allsolid: true,
		endpos: [end[0], end[1], end[2]],
		plane: { normal: vec.emptyV3() } as Plane,
		startsolid: false,
		ent: null,
		inwater: false,
		inopen: false
	}
	var offset = vec.emptyV3();
...
export const move = function (start: V3, mins: V3, maxs: V3, end: V3, type: number, passedict: Edict) {
	var clip: Clip = {
...
export const spawnServer = async function (server: string) {
	...
	for (i = 0; i < def.max_edicts; ++i) {
		...
		_ed = {
			...
			freetime: 0.0,
			v: entV,
			v_float: new Float32Array(entV),
			v_int: new Int32Array(entV),
			visframe: 0
		};

		const float = new Float32Array(_ed.v);
		if (i === 1) {
			const handler = {
			...
			_ed.v_float = new Proxy(float, handler)
		} else {
			_ed.v_float = float
		}
