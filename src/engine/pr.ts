...
export const checkEmptyString = async function (s: string) {
...
export const runError = async function (error: string) {
...
const enterFunction = async function (f: Function) {
  state.stack[state.depth++] = [state.xstatement, state.xfunction];
  var c = f.locals;
...
export const executeProgram = async function (fnum: number) {
  ...
  for (; ;) {
    ...
    switch (st.op) {
      ...
      case OP.address:
        ...
        if ((edictNum === 0) && (sv.state.server.loading !== true))
