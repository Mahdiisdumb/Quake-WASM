...
export const loadModel = function(mod: UnloadedModel | Model, crash: boolean)
{
  ...
  case 0x33504449: // 'IDP3' (Quake III md3)
    loadMD3Model(buf);
    break;
  case 0x46546c67: // 'glTF' (glTF-binary .glb container)
    loadmodel.type = TYPE.alias;
    gltf.loadGLBModel(loadmodel, buf);
    break;
  case 0x50534449:
    loadSpriteModel(buf);
    break;
...
// Real vertex bounds across every pose (QSS Mod_CalcAliasBounds, gl_model.c:3272) — the
// vanilla +/-16 box drops large statics into solid leaves at efrag time (never rendered).
const calcAliasBounds = function()
{
  var mins: V3 = [Infinity, Infinity, Infinity], maxs: V3 = [-Infinity, -Infinity, -Infinity];
  var i, j, k, c;
  const scanPose = function(verts: {v: number[]}[]) {
    for (j = 0; j < verts.length; ++j) {
      for (k = 0; k < 3; ++k) {
        c = verts[j].v[k] * loadmodel.scale[k] + loadmodel.scale_origin[k];
  ...
  for (i = 0; i < loadmodel.numframes; ++i) {
    const f = loadmodel.frames[i];
    if (f.group === true) {
      for (const sub of (f as AliasFrameGroup).frames)
        scanPose(sub.v);
    } else
      scanPose((f as AliasFrame).v);
  }
  if (mins[0] <= maxs[0]) {
    loadmodel.mins = mins;
    loadmodel.maxs = maxs;
...
export const loadAliasModel = function(buffer: ArrayBuffer)
{
  ...
  // Vanilla's alias box, kept for setmodel (QSS mod->clipmins); real render bounds
  // are computed from the frame verts below (QSS Mod_CalcAliasBounds).
  loadmodel.clip_mins = [-16.0, -16.0, -16.0];
  loadmodel.clip_maxs = [16.0, 16.0, 16.0];
