...
const parseNewPeerMessage = (text: string): NewPeerMessage => {
	...
	// Remaining parts are relay URLs (filter out empty strings from final null terminators)
	const relayStrings = parts.slice(1).filter(line => line.length > 0)

	return {
		clientAddress,
		iceServers: relayStrings.length > 0 ? relayStrings.map(parseRelay) : []
	}
}
