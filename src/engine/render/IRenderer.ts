...
export interface IRenderer {
  ...
  // CSQC 3D (world-space) polygons — FTE's cl_stris scene triangles (pr_csqc.c:1508-1521), drawn
  // inside the 3D pass at the end of the scene. Reads the CPU-side batch r.scenePolygon filled off
  // r.state. Alpha-blended, depth-tested but not depth-writing; a twosided batch draws unculled.
  ...
  // w/h are separate because CSQC drawcharacter takes a size VECTOR; rgba modulates the glyph
  // (engine text passes white).
  ...
  // A pic sub-rect stretched into an explicit w/h box with an rgba modulate — CSQC drawpic/drawsubpic
  // (QSS Draw_SubPic). s1/t1/s2/t2 are 0..1 fractions of the pic.
  ...
  // Solid rect in an arbitrary rgba (floats 0-1) — CSQC drawfill.
  drawFillRGBA(x: number, y: number, w: number, h: number, r: number, g: number, b: number, a: number): void
  // One 2D triangle FAN, drawn immediately — CSQC's 2D polygon path (FTE pr_csqc.c:1560-1690).
  // `verts` holds `count` vertices of [x, y, z, u, v, r, g, b, a] — the one format both csqc polygon
  // paths use; x/y are pixels, colours floats 0-1, z ignored. The backend expands the fan into
  // count-2 triangles. pic null = untextured.
  draw2DPolygon(pic: Pic | null, verts: Float32Array, count: number): void
  // Clip subsequent 2D draws to a pixel rect, top-left origin (CSQC drawsetcliparea; the backend owns
  // the y-flip). Cleared by clear2DScissor and, unconditionally, at frame end — a leaked clip must
  ...
  // Re-apply the current gl_texturemode magnification (texture.state.filter_max) to world/model/sprite
  // textures. WebGL2 no-ops — texture.refilterAll already texParameter'd each texture in place. WebGPU
  // swaps its world sampler between point and linear mag (its samplers are immutable, baked into bind
  // groups) and clears the affected caches so they rebuild against the new sampler on the next frame.
